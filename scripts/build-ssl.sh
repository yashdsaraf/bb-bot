...
modpath='PATH=$toolc/bin:$PATH LD_LIBRARY_PATH=$toolc/lib'

for toolc in $ARMMUSL $X86MUSL $MIPSMUSL $MIPSEB
do
	_host=$(find $toolc/bin -name "*rorschack*gcc" | sed 's/.*\///;s/-gcc//')

	eval $modpath CC=$_host'-cc' CFLAGS="-Os -static -fomit-frame-pointer -falign-functions=1 \
	...
	make clean
	eval $modpath make -j$CORES

	cd ssl_helper-wolfssl

	eval $modpath $_host'-gcc' -Os -Wall -I.. -c ssl_helper.c -o ssl_helper.o
	eval $modpath $_host'-gcc' -static -Wl,--start-group ssl_helper.o -lm ../src/.libs/libwolfssl.a -Wl,--end-group -o ssl_helper
	eval $modpath $_host'-strip' ssl_helper
	[[ $_host =~ 86 ]] && _host=x86
